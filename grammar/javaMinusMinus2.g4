...
program: importDecl* mainClass classDecl* interfaceDecl* EOF;
...
classDecl:
	...
	)? '{' fieldDecl* ctorDecl? methodDecl* abstractMethodDecl* '}';

interfaceDecl:
	'interface' Identifier '{' interfaceFieldDecl* interfaceMethodDecl* '}';

interfaceMethodDecl: (type | 'void') Identifier '(' parameterList? ')' ';';

interfaceFieldDecl: type Identifier EQ expression ';';

fieldDecl: varDecl;

localDecl: type Identifier (EQ expression)? ';';

varDecl: accessModifier? type Identifier ';';
...
methodBody: (localDecl | statement)* (RETURN expression? ';')?;

type: (javaType | Identifier) (LSB RSB)?;

javaType: 'boolean' | 'int' | 'char' | 'String';
...
forInit: localDeclNoSemi | assignment (',' assignment)*;

forUpdate: assignment (',' assignment)*;

localDeclNoSemi: type Identifier (EQ expression)?;

printStmt: 'print' LP expressionOrString RP ';';
...
designator: primaryDesignator designatorPrime;

primaryDesignator: Identifier;

designatorPrime: ( '.' Identifier | LSB expression RSB)*;

expressionOrString: expression | StringLiteral;

/* -------------------- Expressions -------------------- */

expression: primaryExpression expressionPrime;
